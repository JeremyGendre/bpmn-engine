...
export default class Engine {
    ...
    try {
      while (
        ![
          ...
          EventType.WAIT,
          EventType.STOP,
        ].includes(this.getLastLog()?.eventType)
      ) {
        ...
        if (!lastLog) {
          ...
          if (resumingElement.type === ElementType.BOUNDARY_EVENT) {
            ...
            const elementAttachedTo =
              processElements[resumingElement.attributes.attachedToRef];
            if (!elementAttachedTo) {
              throw new BPMNError(
                `Can't resume process : attached ref element with id ${resumingElement.attributes.attachedToRef} not found for boundary event with id ${resumingElement.attributes.id}`,
              );
            }
            this.addLog(
              EventType.START_ACTIVITY,
              resumingElement.attributes.id,
            );
          } else {
          ...
          switch (lastElement.type) {
            case ElementType.START_EVENT: {
              if (lastLog.eventType === EventType.END_ACTIVITY) {
                const nextStep = this.getTypedElementOrThrow(
                  lastElement["bpmn:outgoing"],
                );
            ...
            case ElementType.SEQUENCE_FLOW: {
              const nextStep = this.getTypedElementOrThrow(
                lastElement.attributes.targetRef,
              );
              this.addLog(EventType.START_ACTIVITY, nextStep.attributes.id);
              break;
            }
            case ElementType.BOUNDARY_EVENT: {
              const attachedElement = this.getTypedElementOrThrow(
                lastElement.attributes?.attachedToRef,
              );
              // end the activity of the attached element
              this.addLog(
                EventType.END_ACTIVITY,
                attachedElement.attributes.id,
              );
              // end itself
              this.addLog(EventType.END_ACTIVITY, lastElement.attributes.id);
            ...
            case ElementType.EXCLUSIVE_GATEWAY: {
              ...
              if (Array.isArray(outgoingFlows)) {
                ...
                for (let i = 0; i < outgoingFlows.length; i++) {
                  ...
                  const sanitizedCondition = sanitizeEval(
                    flow["bpmn:conditionExpression"]["#text"],
                  );
                ...
                if (!chosenFlow) {
                  ...
                  if (defaultFlowId) {
                    chosenFlow = this.getTypedElementOrThrow(
                      defaultFlowId,
                    ) as TypedSequenceFlow;
                  }
                }
                ...
                if (chosenFlow) {
                  ...
                  throw new BPMNError(
                    `No conditionnal flow taken and no default flow found for exclusive gateway ${lastLog.id}`,
                  );
                }
              } else {
  ...
  async resumeWithId(id: string, result?: any): Promise<State> {
    ...
    if (!resumingElement) {
      throw new BPMNError(
        `Can't resume process : element with id ${id} not found`,
      );
    }
    ...
    if (
      resumingElement.type === ElementType.BOUNDARY_EVENT &&
      resumingElement.attributes.attachedToRef !== this.state.lastActivity
    ) {
      throw new BPMNError(
        `Can't resume process : attached ref element with id ${resumingElement.attributes.attachedToRef} does not correspond to the last process activity ${this.state.lastActivity}`,
      );
    } else if (resumingElement.attributes.id !== this.state.lastActivity) {
      throw new BPMNError(
        `Can't resume process : resuming element ${id} does not correspond to the last process activity ${this.state.lastActivity}`,
      );
    }
